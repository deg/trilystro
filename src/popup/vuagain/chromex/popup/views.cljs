  ...
  (fn []
    [:div
     ...
     [:div {:class "container"}
      ...
      [:form {:class "spaPage", :id "anonymousForm", :style {:display "none"}}
      ...
      [:form {:class "spaPage", :id "loginForm", :style {:display "none"}}
      ...
      [:form {:class "spaPage", :id "TOSForm", :style {:display "none"}}
      ...
      [:form {:class "spaPage", :id "vaWaiting", :style {:display "none"}}
       [:p {:class "warning", :id "waiting"} "Waiting for server..."]]
      [:form {:class "spaPage", :id "vaForm", :style {:display "none"}}
       ...
       [:div {:class "form-group"}
        [:label "Private Note"]
        [:input {:dir "auto", :placeholder "Note", :name "privateNote", :type "text", :maxLength "140", :id "privateNoteInput", :class "form-control", :autoFocus true, :autoComplete "off"}]]
       [:div {:class "form-group"}
        [:label "Public Comment"]
        [:input {:class "form-control", :id "publicCommentInput", :type "text", :placeholder "Comment", :name "publicComment", :autoComplete "off", :dir "auto", :maxLength "140"}]]
       [:label "Rating"]
     ...
     [:div {:class "panel-footer", :style {:margin-top "15px"}}
      [:small
       [:form {:id "chooseServer", :style {:display "none"}}
